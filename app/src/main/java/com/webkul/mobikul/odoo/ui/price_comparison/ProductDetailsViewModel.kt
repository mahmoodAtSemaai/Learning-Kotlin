...
import com.webkul.mobikul.odoo.ui.splash.SplashState
...
class ProductDetailsViewModel @Inject constructor(
	...
	private suspend fun removeFromWishList() {
		isUserLoggedInUseCase(userEntity).collect {
			when (it) {
				is Resource.Default -> _state.value = ProductDetailsState.Idle
				is Resource.Failure -> _state.value = ProductDetailsState.ShowGuestUserLoginDialog
							...
							intent.collect {
								productDetailsState = when (it) {
									...
									is Resource.Success -> {
										...
										_effect.send(
											ProductDetailsViewEffect.RemovedItemFromWishList(
												it.value
											)
										)
	...
	private suspend fun addToCart(product: ProductEntity?, position: Int?) {
		isUserLoggedInUseCase(userEntity).collect {
			when (it) {
				is Resource.Default -> _state.value = ProductDetailsState.Idle
				is Resource.Failure -> _state.value = ProductDetailsState.ShowGuestUserLoginDialog
